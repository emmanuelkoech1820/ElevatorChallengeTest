...
namespace ElevatorChallenge.Core.Implementations
{
    public class ControlService : IControlService
    {
        private readonly List<IElevatorManager> _elevators = new List<IElevatorManager>();
        ...
        public ControlService(IOptions<ElevatorConfiguration> config)
        {
            _config = config.Value;
            for (int i = 0; i < _config.TotalElevators; i++)
            {
                _elevators.Add(new Elevatormanager(i, _config));
            }
        }
        ...
        private Task<IElevatorManager> GetClosestElevator(PassengerRequest request)
        {
            ...
            IElevatorManager closestElevator;
        ...
        public Task<IEnumerable<IElevatorManager>> GetElevators()
        {
            return Task.FromResult<IEnumerable<IElevatorManager>>(_elevators);
        }
